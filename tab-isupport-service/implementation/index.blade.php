...
@section('content')
    ...
    <div class="">
        ...
        <p class="title-text"><i class="bi bi-inboxes-fill" style="color: #dc3545"></i> Implementation</p>
        ...
        <div class="datatable_custom_style">
            <div class="datatable_custom_borderbox">
                <div class="table-btn-container d-flex justify-content-start gap-2">
                    ...
                    {{--  <a class="btn btn-success" href="{{ route('tab-isupport-service.implementation.print') }}" role="button" target="_blank">Print</a> --}}
        
                    <div id="printButtonContainer" class="dt-buttons"></div>
        
                    <a class="main-btn" href="{{ route('tab-isupport-service.implementation.create') }}" role="button"><i
                            class="bi bi-plus-lg"></i> New
                        Project/Opportunity
                    </a> 
                        
                    @if(Route::currentRouteName() === 'tab-isupport-service.implementation.index' && $isAllowedToEdit)
                    <div class="vr"></div>
                    <button type="button" class="btn btn-danger softdelete-button" disabled>
                    ...
                    <table id="implementation-datatable" class="basic-border nowrap" style="width: 100%;">
                        <thead>
                            <tr>
                                @if(Route::currentRouteName() === 'tab-isupport-service.implementation.index' && $isAllowedToEdit)
                                <th style="text-align: center; vertical-align: middle;"><input type="checkbox" class="row-checkbox" id="selectAll"></th>
                                @endif
                                <th>Project Code</th>
                                <th>Project Name</th>
                                <th>Date Filed</th>
                                ...
                                <th class="d-none">Id</th> <!-- Hidden column for project list ID -->
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            @if (isset($implementationProjects))
                                @foreach ($implementationProjects as $project)
                                    <tr data-cashreqstatus="{{ optional($project->latest_cash_request_status)->status }}" data-createdby="{{ $project->created_by }}" data-proj="{{ $project->proj_code }}"
                                        data-id="@foreach ($project->getTeamMember as $teamMemberID){{ $teamMemberID->project_id }}, @endforeach"
                                        data-type="{{ optional($project->project_type)->name }}" data-manager="{{ $project->PM }}"
                                        data-members="@foreach ($project->getTeamMember as $teamMember){{ $teamMember->eng_name }}, @endforeach"
                                        data-value="{{ optional($project->cashAdvance)->cash_advance . '|' . $project->rs_contact . '|' . $project->rs_email . '|' . $project->endUser . '|' . $project->eu_contact . '|' . $project->eu_email . '|' . $project->project_net . '|' . $project->manday_cost }}"
                                        data-euloc="{{ $project->eu_location }}"
                                        data-attachments="@foreach ($project->proj_attachments as $projAttachments){{ $projAttachments->attachment }}, @endforeach"
                                        data-signoffattachments="@foreach ($project->signOff_attachments as $signOffAttachments){{ $signOffAttachments->attachment }}, @endforeach"
                                        data-cashadvanceattachments="@foreach ($project->cashAdvance_attachments as $cashAttachments){{ $cashAttachments->attachment_file }}, @endforeach">
                                        ...
                                        {{-- <td class="doer_eng">
                                            @if ($project->doer_engineers && $project->doer_engineers->isNotEmpty())
                                                {!! implode('<br><br>', $project->doer_engineers->pluck('eng_name')->toArray()) !!}
                                            @else
                                                No Doers/Engineers
                                            @endif
                                        </td> --}}
                                        ...
                                        {{-- <td class="special_ins">{!! nl2br($project->special_instruction) !!}</td> --}}
                                        ...
                                        <td class="d-none proj_id">{{ $project->id }}</td> <!-- Hidden column value -->
                                    </tr>
                                @endforeach
                            @endif
                        </tbody>
