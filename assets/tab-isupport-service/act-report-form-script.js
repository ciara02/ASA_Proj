...
$(document).ready(function () {
    ...
    $('#datatable1 tbody').on('click', 'tr', function () {
        ...
        ajaxRequests.push($.ajax({
            ...
            data: { refNo: referenceNumber },
            success: function (response) {
                console.log('Response from server ProductLine: ', response);

                var productlineDropdown = $('#product_line');
                productlineDropdown.empty(); // Clear existing options

                if (response) {
                    // Parse JSON response
                    var productlines = response.split(','); // Assuming product lines are comma-separated
                ...
                productlineDropdown.trigger('change'); // Update the Select2 dropdown
            },
            complete: trackProgress
        }));
...
document.addEventListener('DOMContentLoaded', function () {
    let category = null; // Initialize as null
    let status = null;   // Initialize as null
    let isreportNameSet = false;
    let isStatusNameSet = false;
    ...
    function checkConditionsAndToggleVisibility() {
        ...
        document.querySelectorAll('#exampleModal .modal-body .card').forEach(card => card.style.display = 'none');
    ...
    // Event listener for when 'reportNameSet' is triggered
    $(document).on('reportNameSet', function () {
        category = document.querySelector('#reportDropdown1').value;
        console.log("reportNameSet event triggered, reportName:", category);
        isreportNameSet = true;

        // Call the function to check conditions and toggle visibility
        checkConditionsAndToggleVisibility();
    });

    // Event listener for when 'statusNameSet' is triggered
    $(document).on('statusNameSet', function () {
        status = document.querySelector('#statusDropdown1').value;
        console.log("statusNameSet event triggered, statusName:", status);
        isStatusNameSet = true;
    ...
    function handleDropdownChange() {
        // Update category or status based on which dropdown changed
        category = document.querySelector('#reportDropdown1').value;
        status = document.querySelector('#statusDropdown1').value;

        console.log('Dropdown change detected. Category:', category, 'Status:', status);

        // Re-check conditions after a dropdown change
        checkConditionsAndToggleVisibility();
    }
});
