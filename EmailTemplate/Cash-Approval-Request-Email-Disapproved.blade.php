...
<head>
    <meta charset="UTF-8">
    <title>Disapproved Request</title>
    <style>
        .title {
            margin-top: 2rem;
            text-align: center;
        ...
        .alert-danger{
            background-color: red;   /* Custom yellow background */
...
<body>
    @if($role === 'requester')
        ...
        <p>
        We regret to inform you that your cash advance request for the project 
        <b>{{ $record->proj_name }}</b> (Project Reference No.: <b>{{ $record->proj_ref_id }}</b>) 
        has been <b>disapproved</b> by <b>{{ $record->approver_name }}</b>. As a result, the request was not recorded in the system.
        </p>
    @elseif($role === 'approver')
        <p>Good day!</p>

        <p>
            You disapproved the cash advance request for the project <b>{{ $record->proj_name }}</b> (Project Reference No.: <b>{{ $record->proj_ref_id }}</b>) submitted by
            <b>{{ $record->requested_by }}</b>. As a result, the request was not recorded in the system.
        </p>
    @endif
    ...
    <div class="row">
        <center>
            ...
            <p class="mt-4 mb-4" style="text-align: left; margin-left: 5%;">
                Status: <span id="statusText" class="alert alert-danger px-2 py-1 m-0" style="background-color: #ff5050;">{{ $record->status }}</span>
            </p>
