...
<head>
    ...
    <style>
        ...
        .alert-success{
            background-color: green;   /* Custom yellow background */
...
<body>
        @if($role === 'requester')
        <p>Hi {{ $record->requested_by }},</p>

        <p>Good day!</p>

        <p>
            We are pleased to inform you that your cash advance request for the project <b>{{ $record->proj_name }}</b> 
            (Project Reference No.: <b>{{ $record->proj_ref_id }}</b>) has been <b>approved</b> by <b>{{ $record->approver_name }}</b>. 
            The request has been successfully recorded in the system.
        </p>
        @elseif($role === 'approver')
            <p>Good day!</p>

            <p>
                You approved the cash advance request for the project
                <b>{{ $record->proj_name }}</b> (Project Reference No.: <b>{{ $record->proj_ref_id }}</b>) submitted by
                <b>{{ $record->requested_by }}</b>. The request has been successfully recorded in the system.
            </p>
        @endif
    
    <p>
        <a href="{{ $viewUrl }}" class="button">Click here to view and print the approved request.</a>
    </p>
    ...
    <div class="row">
        <center>
            ...
            <p class="mt-4 mb-4" style="text-align: left; margin-left: 5%;">
                Status: <span id="statusText" class="alert alert-success px-2 py-1 m-0">{{ $record->status }}</span>
            </p>
